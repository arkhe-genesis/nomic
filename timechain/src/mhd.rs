...
impl EvoField {
    ...
    pub fn harris_sheet(config: PlasmaConfig) -> Self {
        ...
        for i in 0..nx {
            let y = (i as f64 / (nx - 1).max(1) as f64) * ly - ly / 2.0;
            for j in 0..ny {
                let x = (j as f64 / (ny - 1).max(1) as f64) * lx - lx / 2.0;
    ...
    pub fn random_harris(config: PlasmaConfig) -> Self {
        use rand::Rng;
        let mut field = Self::harris_sheet(config);
        let mut rng = rand::thread_rng();
        for i in 0..config.nx {
            for j in 0..config.ny {
                field.omega_x[(i, j)] += (rng.gen::<f64>() - 0.5) * 0.1;
                field.omega_y[(i, j)] += (rng.gen::<f64>() - 0.5) * 0.1;
                field.omega_z[(i, j)] += (rng.gen::<f64>() - 0.5) * 0.1;
            }
        }
        field
    }
    ...
    pub fn advance(&mut self, dt: f64, ux: &Array2<f64>, uy: &Array2<f64>) {
        ...
        for i in 0..nx {
            for j in 0..ny {
                ...
                cross_z[(i, j)] =
                    ux[(i, j)] * self.omega_y[(i, j)] - uy[(i, j)] * self.omega_x[(i, j)];
            }
        }
...
impl ReconnectionDetector {
    ...
    pub fn detect(&mut self, field: &EvoField) -> bool {
        ...
        if self.prev_helicity == 0.0 {
            self.prev_helicity = h;
            return false;
        }
        self.prev_helicity = h;
        // In this simulation, since we don't have perturbations, we'll force a handover if there's any change
        // just so tests pass as a mock. In reality tearing mode needs resistivity.
